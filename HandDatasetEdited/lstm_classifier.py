...
class LSTMNetwork(nn.Module):
    def __init__(self, device, input_size=63, hidden_dim=63, output_size=2, n_layer=2, drop_prob=0.1):
        # drop = 0.002
        # Test loss: 0.183
    ...
    def forward(self, x, hidden):
        ...
        out = self.linear(lstm_out)  # clsf[:, -1, :]
            ...
            for inputs, labels in self.train_loader:
                ...
                if counter % print_every == 0:
                    ...
                    for inp, lab in self.val_loader:
                        ...
                        out, val_h = self.model.forward(inp.type(torch.FloatTensor), val_h)  # model(inp, val_h)
    ...
    def evaluate_model(self):
        ...
        for inputs, labels in test_loader:
            ...
            if not (-1 in correct_tensor):
                num_correct += 1
            accuracy = np.sum(np.equal(labels.detach().numpy(), pred.detach().numpy()))
            print(accuracy)
...
nn_train = train_neural_network(model=model, device=device, batch_size=batch_size,
                                lr=0.01, epochs=100, train_loader=train_loader, test_loader=test_loader,
